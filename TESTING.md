...
The Miri and AddressSanitizer lanes retain the established C3 alias/lifetime suite, execute the controlled worker-projection test module, and execute the deterministic cohort-executor conformance module. Together those suites exercise real scoped-thread execution over prepared disjoint World projections, exact shared/mutable payload bounds, metadata-only projections, deterministic mutation-journal reconciliation, worker-local deferred recording, semantic cohort boundaries, canonical deferred publication, invoker-thread fences, failure selection, invariant priority, callback fail-stop behavior, and Runtime reuse. The public serial executor remains the independent correctness oracle for the supported parallel selector.
...
deterministic cohort-executor conformance module with
...
and the supported production executor, including cohort join/drain,
capacity-admission, and failure-handling paths. TSan remains supplemental
evidence rather than exhaustive interleaving exploration.
